...
document.getElementById('btn-clear-history').addEventListener('click', async () => {
    const confirmClear = confirm("Are you sure you want to completely clear your gaming history?");
    if (confirmClear) {
        const success = await window.api.clearHistory();
        if (success) {
            await loadGames();
            alert("Gaming history cleared.");
        }
    }
});
...
document.getElementById('btn-open-manual').addEventListener('click', () => { document.getElementById('modal-about').classList.remove('active'); floatingManual.style.display = 'flex'; });
...
function renderTable(recent, regular) {
    ...
    const appendRow = (game) => {
        ...
        tr.innerHTML = `
        <td>${game.LaunchCommand ? `<button class="primary btn-play" data-cmd="${game.LaunchCommand.replace(/"/g, '&quot;')}" data-id="${game.id}" style="padding: 4px 8px;">Play</button>` : '<span style="color:#555; font-size:12px;">No Cmd</span>'}</td>
    ...
    if (recent && recent.length > 0) {
        ...
        trLabel.innerHTML = `<td colspan="7" style="background: var(--bg_menu); color: var(--accent); font-weight: 900; letter-spacing: 2px; text-align: center;">RECENT GAMES</td>`;
        tbody.appendChild(trLabel);
        recent.forEach(appendRow);

        const trAll = document.createElement('tr');
        trAll.innerHTML = `<td colspan="7" style="background: var(--bg_menu); color: var(--text_sec); font-weight: 900; letter-spacing: 2px; text-align: center;">ALL GAMES</td>`;
...
function renderGallery(recent, regular) {
    ...
    const appendCard = (game) => {
        ...
        const imgHtml = imgSrc ? `<img src="${imgSrc}" class="gallery-cover">` : `<div class="gallery-cover" style="display:flex; align-items:center; justify-content:center; color:#555; font-size:12px;">NO COVER</div>`;
        div.innerHTML = `
        ${imgHtml}
        <div class="gallery-title">${game.Game}</div>
        ${game.LaunchCommand ? `<button class="btn-play-gallery primary" data-cmd="${game.LaunchCommand.replace(/"/g, '&quot;')}" data-id="${game.id}" style="margin: 5px; font-size: 12px; padding: 4px;">Play</button>` : ''}
        `;
...
function openGamepage(game) {
    ...
    const updateTogglesUI = () => {
        ...
        favBtn.innerText = game.FAV === 'YES' ? '★ FAV' : '+ FAV';

        wantBtn.classList.toggle('active', game.WANT_TO_PLAY === 'YES');
        wantBtn.innerText = game.WANT_TO_PLAY === 'YES' ? '⚑ WANT TO PLAY' : '+ WANT TO PLAY';
    };
    updateTogglesUI();
    ...
    if (game.Description && game.Description.trim() !== "") {
        shortDescContainer.innerText = game.Description;
    ...
    if (game.SteamDesc && game.SteamDesc.trim() !== "") {
        ...
        fallbackDescContainer.innerText = "No detailed description available.";
        // Only show fallback if we also didn't have a short description
        fallbackDescContainer.style.display = (game.Description && game.Description.trim() !== "") ? 'none' : 'block';
    }
...
document.getElementById('btn-delete-trailer').addEventListener('click', async () => {
    ...
    if (success) alert("Trailer deleted."); else alert("No trailer found to delete.");
});
...
document.getElementById('btn-clear-meta').addEventListener('click', () => {
    ...
    document.getElementById('edit-tags').value = ""; document.getElementById('edit-similar').value = "";
...
async function openSgdbModal(apiKey, assetType, manualQuery) {
    ...
    grid.innerHTML = '';
    stat.innerText = `Searching SteamGridDB for ${assetType.toUpperCase()}...`;
    ...
    if (results.length === 0) { stat.innerText = "No artwork found. Try a manual search."; return; }

    stat.innerText = `Select a Custom ${assetType.toUpperCase()}:`;
    results.forEach(res => {
        ...
        img.addEventListener('click', async () => {
            stat.innerText = "Downloading selected artwork...";
            ...
            if (newPath) {
                ...
                stat.innerText = "❌ Download failed.";
            }
...
document.getElementById('btn-delete-game').addEventListener('click', async () => {
    if (!currentGameId) return;
    const confirmDelete = confirm("Are you sure you want to permanently delete this game?");
    if (confirmDelete) {
...
function triggerAutoFetchSearch(gameId, gameName) {
    ...
    window.api.searchSteam(gameName).then(results => {
        if (results.length === 0) {
            ...
            btn.innerText = "✨ AUTO-FETCH"; btn.disabled = false; return;
        }
        if (results.length === 1) {
            ...
            btn.innerText = "✨ AUTO-FETCH"; btn.disabled = false;
        }
    });
}
...
function openSteamResultsModal(gameId, gameName, results) {
    ...
    results.forEach(res => {
        ...
        btn.addEventListener('click', () => {
            ...
            document.getElementById('btn-auto-fetch').innerText = "⏳ FETCHING...";
...
document.getElementById('btn-fetch-proton').addEventListener('click', async () => {
    ...
    if (!appId) { alert("Steam App ID is required to fetch ProtonDB status."); return; }
...
document.getElementById('btn-watch-trailer').addEventListener('click', async () => {
    ...
    if (localUrl) {
        ...
        lst.innerHTML = ''; stat.innerText = `Searching YouTube for "${gameName}"...`;
        const results = await window.api.searchYoutube(gameName);
        if (results.length === 0) { stat.innerText = "No trailers found on YouTube."; return; }
        stat.innerText = "Select a video to download:";
...
function openTrailerProgress(gameName, videoId) {
    ...
    window.api.downloadTrailer(gameName, videoId).then(success => {
        ...
        if (success) {
            alert("Trailer download complete!");
            // Refresh logic to pick up the new video file could go here
        } else {
            alert("Trailer download failed.");
        }
    });
}
...
document.getElementById('btn-sync-heroic').addEventListener('click', async () => {
    ...
    btn.innerText = "Sync Heroic Games";
});
...
document.getElementById('btn-sync-steam').addEventListener('click', async () => {
    ...
    if (!steamId || !apiKey) { alert("Please enter both your SteamID and Steam API Key."); return; }
    ...
    alert(result.message);
    if (result.success) loadGames();
    btn.innerText = "Fetch Steam Library"; btn.disabled = false;
});

document.getElementById('btn-sync-gog').addEventListener('click', async () => {
    const btn = document.getElementById('btn-sync-gog');
    btn.innerText = "⏳ Waiting for Login..."; btn.disabled = true;
    const result = await window.api.syncGog();
    alert(result.message);
    if (result.success) loadGames();
    btn.innerText = "Fetch GOG Library"; btn.disabled = false;
});

document.getElementById('btn-clear-data').addEventListener('click', async () => {
    const confirmed = confirm("Are you sure you want to completely wipe all browser data, cookies, and active store logins?");
...
document.getElementById('btn-clean-images').addEventListener('click', async () => {
    const confirmed = confirm("This will scan your images folder and delete any orphaned files not currently used by a game in your library. Proceed?");
    if (confirmed) {
...
document.getElementById('btn-clear-all-images').addEventListener('click', async () => {
    const confirmed = confirm("☢️ WARNING: This will completely wipe ALL downloaded artwork and screenshots, resetting your library visually. Are you absolutely sure?");
    if (confirmed) {
...
document.getElementById('btn-restore-zip').addEventListener('click', async () => {
    const confirmed = confirm("Warning: Restoring a backup will overwrite your current library. Are you sure you want to proceed?");
    if (confirmed) {
...
document.getElementById('btn-batch-fetch').addEventListener('click', async () => {
    ...
    if (gamesToFetch.length === 0) { document.getElementById('batch-status').innerText = "All games are fully up to date!"; return; }
    ...
    for (let i = 0; i < gamesToFetch.length; i++) {
        ...
        statusText.innerText = `Fetching ${i + 1} / ${gamesToFetch.length}: ${game.Game}...`;
        await window.api.autoFetch(game.id, game.Game, game.SteamAppID);
        await new Promise(resolve => setTimeout(resolve, 500));
    }
    statusText.innerText = `✅ Finished fetching ${gamesToFetch.length} games!`; btn.disabled = false; loadGames();
});
...
document.getElementById('btn-add-game').addEventListener('click', async () => {
    ...
    } else { alert("Failed to create a new game entry."); }
});
...
document.getElementById('btn-import-csv').addEventListener('click', async () => {
    ...
    btn.innerText = "📥 Import CSV"; btn.disabled = false;
});
...
function updateHeroMosaic(filtered, filterName) {
    ...
    const filterMap = {
        'all': { text: 'ALL GAMES', icon: 'all_games' }, 'playable': { text: 'PLAYABLE', icon: 'playable' },
    ...
    if (countEl) countEl.innerText = `${filtered.length} ${filtered.length === 1 ? 'GAME' : 'GAMES'}`;
...
const THEMES = {
    ...
    "WIN XP": {bg: "#003399", bg_panel: "rgba(236, 233, 216, 0.2)", bg_menu: "#0054E3", accent: "#3CA02B", accent_menu: "#3CA02B", text_main: "#FFFFFF", text_sec: "#ECE9D8", text_dim: "#99B4D1", border: "rgba(236, 233, 216, 0.4)", border_solid: "#ECE9D8"},
...
function renderThemeCategories() {
    themeTitle.innerText = "Theme Categories";
